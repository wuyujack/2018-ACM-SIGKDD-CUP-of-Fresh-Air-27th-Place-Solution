...
clear
LD=readtable('ld_meo_2018-05-31.csv');%a按站点顺序，不用删除天气状况和风向，删去station_id
...
%%%导出csv以后记得去删除空白行
tic
writetable(LD_temperature,'LD_5_31_with_location.csv','Delimiter',',','QuoteStrings',true)
toc
...
clear
tic
LD=readtable('LD_5_31_with_location_new.csv');%%%导入伦敦历史气象网格数据，上传前要按时间排序，不用删除任何行，一共11行
...
tic
writetable(London_station_data_aqi,'London_5_31_get_test_data.csv','Delimiter',',','QuoteStrings',true)
toc
...
clear
tic
LD_aqi_forecast=readtable('ld_aqi_2018-05-31.csv');%%%导入伦敦历史气象网格数据，通过api获取以后，保留站点、经纬度、PM2.5、PM10、NO2
...
London_station_data_aqi=readtable('London_5_31_get_test_data_new.csv');%%注意要只保留aqi列为true的表格，也就是第一列为true
...
%%%导出csv以后记得去删除空白行
tic
writetable(LD_temperature,'LD_5_31_test_full_result.csv','Delimiter',',','QuoteStrings',true)
...
temp_LD_data_new_new=cell(size(LD_data,1),36,size(LD_data,3));
tic
for i=1:size(temp_LD_data_new,3)
         i 
         if ~isnan(cell2mat(temp_LD_data_new(1,3,i)))
        temp_LD_data_new_new(:,1:36,i)=LD_knn_aqi_station(temp_LD_data_new(:,:,i));
        end
end
toc
...
%%%%%导出csv文件删除空行
tic
writetable(LD_temperature_new,'5_31_LD_data_75_model_full_result.csv');
toc

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear
tic
LD=readtable('5_31_LD_data_75_model_full_result_new_without_sort.csv');%%%按aqi站点排序
toc
...
%%%记得修改保存文件名称
tic
writetable(LD_temperature,'LD_aqi_meo_5_31_75_model_test_new.csv','Delimiter',',','QuoteStrings',true)
toc