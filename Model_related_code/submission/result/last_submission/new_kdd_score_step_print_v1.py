...
true = get_true('2018-05-12 00:00:00')
sub2 = pd.read_csv(r'F:\5_12_6\result_aqi48_75_include_April_v2.csv')
...
f = open(r'F:\result_aqi48_75_include_April_v2.txt','w')

#按小时输出结果（i代表前i小时）
print('hour,','result_item,','PM2.5,','PM10,','O3,',file=f)
for i in range(0,48):
    true_bj=true[(true['hour'] <= i) & (true['city']=='bj')]
    smape_pf_sub(true_bj,sub2)
    print('hour %s,'%i,'%s,'%'bj',smape_pf_sub(true_bj,sub2)[0],',',smape_pf_sub(true_bj,sub2)[1],',',smape_pf_sub(true_bj,sub2)[2],',',file=f)
    true_ld=true[(true['hour'] <= i) & (true['city']=='ld')]
    smape_pf_sub(true_ld,sub2)
    print('hour %s,'%i,'%s,'%'ld',smape_pf_sub(true_ld,sub2)[0],',',smape_pf_sub(true_ld,sub2)[1],',',file=f)
    jg2 = smape_pf_sub(true_bj,sub2)
    jg1 = smape_pf_sub(true_ld,sub2)
