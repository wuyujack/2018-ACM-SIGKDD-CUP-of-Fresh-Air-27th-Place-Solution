...
num_of_variable =[145,176][location == 'BJ']#beiing
...
def recall_aqi(location,data,model_index):
    model_dir = "/home/dedekinds/newlightgbm"
    model_name = location+'model'+str(model_index)+'.txt'#BJmodel179.txt
    model = joblib.load(os.path.join(model_dir, model_name))
    temp_ans = 0
    for mod in model:
        temp_ans+=mod.predict(data, num_iteration=mod.best_iteration)
...
print(pd_data)
model_dir = "result"
model_name = 'result_'+location+'_weather_new.csv'
