...
%W=1;
    ...
    for i=1:size(d,1)
        for  j=1:size(temp_meo_new,1) 
            d(i,j+6)=sqrt((d(i,1)-temp_meo_new(j,1))^2+(d(i,2)-temp_meo_new(j,2))^2);%欧式距离=根号（（x1-x2）^2+(y1-y2)
        end
        temp(i,1:size(temp_meo_new,1))=sort(d(i,7:size(temp_meo_new,1)+6));%%对距离进行排序
    end

    %%%%求出18个气象观测站在u、v方向上面风速的分量
    u=zeros(size(temp_meo_new,1),1); %初始化u方向上的风速
    v=zeros(size(temp_meo_new,1),1); %初始化v方向上的风速
    for j = 1:size(temp_meo_new,1) %对于输入的某个时刻的气象站的风速
        if isnan(temp_meo_new(j,7)) %如果该行对应的气象站的风速为NaN
        ...
        elseif temp_meo_new(j,7)>=999016
            ...
            u(j) = temp_meo_new(j,8) * sind((temp_meo_new(j,7)));%u方向分量
            v(j) = temp_meo_new(j,8) * cosd((temp_meo_new(j,7)));%v方向分量
        end
    end

    k=4; %近邻数大于等于2
    h=2; %反距离的距离的幂数
    W=cell(size(d,1),3,1);%以35个空气质量站数目构造一个三维数组
    ...
    for i=1:size(d,1)%对35个空气质量站分别求风向和风速
        ...
        for l=2:k %k是规定的选择的最大近邻的气象观测站的个数
            ...
            W(i,3,1)=num2cell(temp_fenzi/temp_fenmu); %根据反向加权求出温度
            W(i,4,1)=num2cell(pressure_fenzi/pressure_fenmu); %根据反向加权求出压强
            W(i,5,1)=num2cell(humidity_fenzi/humidity_fenmu); %根据反向加权求出湿度
            W(i,6,1)=num2cell(ufenzi/ufenmu); %根据反向加权求出u方向的风速
            W(i,7,1)=num2cell(vfenzi/vfenmu); %根据反向加权求出v方向的风速
            W(i,8,1)=num2cell(sqrt(cell2mat(W(i,6))^2+cell2mat(W(i,7))^2)); %利用三角形法则(勾股定理)合成风速
    end
