...
function temp_LD_data_new_1=LD_knn_full_aqi_station(temp_LD_data)
%%%计算temp_LD_data的非空行数
 ...
 temp_LD_data_new_1=temp_LD_data(:,1:12);
    ...
    for i=1:LD_empty%对35个空气质量站分别
       ...
       for l=3:k+2 %k是规定的选择的最大近邻的气象观测站的个数,由于第1个距离是0，因此要从第二开始
                        ...
                        temp_LD_data_new_1(i,13+(j-1)*9)=temp_LD_data(n,1);%站点名称
                        temp_LD_data_new_1(i,14+(j-1)*9)=temp_LD_data(n,5);%PM2.5
                        temp_LD_data_new_1(i,15+(j-1)*9)=temp_LD_data(n,6);%PM10
                        temp_LD_data_new_1(i,16+(j-1)*9)=temp_LD_data(n,7);%O3
                        temp_LD_data_new_1(i,17+(j-1)*9)=temp_LD_data(n,8);%风速
                        temp_LD_data_new_1(i,18+(j-1)*9)=temp_LD_data(n,12);%风向
                        temp_LD_data_new_1(i,19+(j-1)*9)=temp_LD_data(n,9);%PM2.5
                        temp_LD_data_new_1(i,20+(j-1)*9)=temp_LD_data(n,10);%PM10
                        temp_LD_data_new_1(i,21+(j-1)*9)=temp_LD_data(n,11);%NO2
                        j=j+1;
       end
       j=1;
    end
